...
public class StatementCamt53_001 implements Statement {
  ...
  public List<StatementDTO> process(File z53OutFile) throws IOException {
    ...
    if (z53OutFile.exists() && z53OutFile.getParentFile().getName().equals("error")) {
      ...
      String unzipDir =
          appConfig.outputDir + "/" + FilenameUtils.removeExtension(z53OutFile.getName());
        ...
        .forEach(
            statement -> {
              ...
              try {
                ...
                Document document =
                    (Document) JAXBIntrospector.getValue(unmarshaller.unmarshal(statement));

                List<Document> documents =
                    processedFiles.get(
                        document.getBkToCstmrStmt().getStmt().get(0).getAcct().getId().getIBAN());
                if (documents == null) {
                  ...
                  stmts
                      ...
                      .forEach(
                          stmt -> {
                            ...
                            // Set Dates
                            XMLGregorianCalendar bookgDt =
                                stmt.getNtry().get(0).getBookgDt().getDt();
                            XMLGregorianCalendar valDt = stmt.getNtry().get(0).getValDt().getDt();
                            builder
                                .bookingDate(
                                    LocalDate.of(
                                        bookgDt.getYear(), bookgDt.getMonth(), bookgDt.getDay()))
                                .validationDate(
                                    LocalDate.of(
                                        valDt.getYear(), valDt.getMonth(), valDt.getDay()));
                                    ...
                                    entry -> {
                                      ...
                                      if (code == CreditDebitCode.CRDT) {
                                        entry
                                            ...
                                            .forEach(
                                                details -> {
                                                  EntryTransaction4 transaction =
                                                      details.getTxDtls().get(0);

                                                  TransactionDTO.TransactionDTOBuilder builderIn =
                                                      TransactionDTO.builder();
                                                  // Prepare addrLine as ArrayList
                                                  ArrayList<String> addrLine = new ArrayList<>();
                                                  if (transaction.getRltdPties() != null) {
                                                    if (transaction.getRltdPties().getDbtr()
                                                        != null) {
                                                      if (transaction
                                                              .getRltdPties()
                                                              .getDbtr()
                                                  ...
                                                  if (transaction.getRefs() != null) {
                                                    builderIn.endToEndId(
                                                        transaction.getRefs().getEndToEndId());
                                                    builderIn.instrId(
                                                        transaction.getRefs().getInstrId());
                                                    builderIn.msgId(
                                                        transaction.getRefs().getMsgId());
                                                    builderIn.pmtInfId(
                                                        transaction.getRefs().getPmtInfId());
                                                  }
                                                  if (transaction.getRmtInf() != null) {
                                                    if (transaction.getRmtInf().getUstrd()
                                                        != null) {
                                                      builderIn.reference(
                                                          transaction
                                                              .getRmtInf()
                                        ...
                                        entry
                                            ...
                                            .forEach(
                                                details -> {
                                                  EntryTransaction4 transaction =
                                                      details.getTxDtls().get(0);
                                                  TransactionDTO.TransactionDTOBuilder builderOut =
                                                      TransactionDTO.builder();

                                                  if (transaction.getRltdPties() != null) {
                                                    if (transaction.getRltdPties().getCdtr()
                                                        != null) {
                                                      builderOut.name(
                                                          transaction
                                                              .getRltdPties()
                                                    ...
                                                    if (transaction.getRltdPties().getCdtrAcct()
                                                        != null) {
                                                      builderOut.iban(
                                                          transaction
                                                              .getRltdPties()
                                                  ...
                                                  builderOut.amount(
                                                      transaction.getAmt().getValue());
                                                  builderOut.currency(
                                                      transaction.getAmt().getCcy());
                                                  if (transaction.getRmtInf() != null) {
                                                    if (transaction.getRmtInf().getUstrd() != null
                                                        && transaction.getRmtInf().getUstrd().size()
                                                            > 0) {
                                                      builderOut.reference(
                                                          transaction
                                                              .getRmtInf()
                                                              .getUstrd()
                                                              .get(0));
