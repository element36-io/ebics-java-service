...
public class EbicsStatementServiceImpl  implements EbicsStatementService {
    ...
    public List<StatementDTO> getBankStatement() throws IOException {
        ...
        if (Files.exists(Paths.get(appConfig.outputDir + "/error"))){
            Files.list(Paths.get(appConfig.outputDir + "/error")).filter(Files::isRegularFile).forEach(file -> {
                try {
                    log.debug(" files in error dir found, move it to processing folder and add processing array {} ",file);
                    errorStatements.addAll(statementStrategy.process(file.toFile()));
                    new EbicsTools().moveFile(file.toFile().getAbsolutePath(), appConfig.outputDir + "/" + file.toFile().getName());
                } catch (Exception e) {
        ...
        File z53OutFile = new File(String.format("%s%s%s%s", appConfig.outputDir, "/z53-", new Date().toInstant().getEpochSecond(), ".zip"));
